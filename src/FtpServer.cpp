...
/*********************************************/
boolean FtpServer::inicializaFTP(boolean debug)
  {
  ...
  if(!recuperaDatosFTP(debugGlobal)) Serial.println("Configuracion del servidor FTP por defecto");
  else
    {
...
boolean FtpServer::recuperaDatosFTP(boolean debug)
  {
  String cad="";
...
/*********************************************/
boolean FtpServer::parseaConfiguracionFTP(String contenido)
  { 
...
void FtpServer::handleFTP()
{
  ...
  else if( readChar() > 0 )         // got response
  {
    if( cmdStatus == 3 )            // Ftp server waiting for user identity
      ...
      {//INCLUYO ESTAS LLAVES PARA EVITAR AMBIGÜEDADES
      if( ! processCommand())
        cmdStatus = 0;
      else
        millisEndConnection = millis() + millisTimeOut;
...
boolean FtpServer::processCommand()
{
  ...
  {
    //VARIABLE NO UTILIZADA char path[ FTP_CWD_SIZE ];
  ...
  {  
    char path[ FTP_CWD_SIZE ];
    //VARIABLE NO UTILIZADA char dir[ FTP_FIL_SIZE ];
